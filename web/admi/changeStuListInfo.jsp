...
<script>
    function deleteUser(sid,currentPage){
        //用户安全提示
        if(confirm("您确定要删除吗？")){
            //访问路径
            location.href="${pageContext.request.contextPath}/delStudentServlet?sid="+sid+"&currentPage="+currentPage;
        }
    }
</script>
<body>
...
<div>
    ...
    <div style="float:left;width: 85%;">
        ...
        <span style="text-align: center; display: block;font-family:黑体;font-size: 1.5em;">学生信息详情</span>
        <div class="blank" style="height: 10px;"></div>
        <div class="container">
            <div style="float: left;margin-top: 20px">
                <form class="form-inline" action="${pageContext.request.contextPath}/changeStuInfoListServlet"
                      method="post">
                    <div class="form-group">
                        <label for="exampleInputName2">学号</label>
                        <input type="text" name="ssno" value="${condition.ssno[0]}" class="form-control"
                               id="exampleInputName2">
                    </div>
                    <div class="form-group">
                        <label for="exampleInputName3">姓名</label>
                        <input type="text" name="sname" value="${condition.sname[0]}" class="form-control"
                               id="exampleInputName3">
                    </div>

                    <div class="form-group">
                        <label for="exampleInputEmail2">专业</label>
                        <input type="data" name="smajor" value="${condition.smajor[0]}" class="form-control"
            ...
            <table class="table table-striped">
                ...
                <c:forEach items="${pb.list}" var="t" varStatus="s">
                    <tr align="center">
                        ...
                        <th>${t.sphone}</th>
                        <th>
                            <input type="submit" value="修改" onclick="window.location.href = '${pageContext.request.contextPath}/returnOneStuInfoServlet?sid=${t.sid}'" style="border:1px solid rgb(154, 240, 154);">
                            <input type="button" value="删除" onclick="window.location.href = 'javascript:deleteUser(${t.sid},${pb.currentPage});'" style="border:1px solid rgb(248, 69, 69);">
                        </th>
                    </tr>
                </c:forEach>
            </table>
        ...
        <div style="margin-left: 15%;margin-bottom: 10%">
            <nav aria-label="Page navigation">
                <ul class="pagination pagination-lg">
                    <li>
                        <a href="${pageContext.request.contextPath}/changeStuInfoListServlet?currentPage=${pb.currentPage -1}&rows=10&ssno=${condition.ssno[0]}&sname=${condition.sname[0]}&smajor=${condition.smajor[0]}"
                           aria-label="Previous">
                    ...
                    <c:forEach begin="1" end="${pb.totalPage}" var="i">
                        <c:if test="${pb.currentPage == i}">
                            <li class="active">
                                <a style="color: #0f0f0f;" href="${pageContext.request.contextPath}/changeStuInfoListServlet?currentPage=${i}&rows=10&ssno=${condition.ssno[0]}&sname=${condition.sname[0]}&smajor=${condition.smajor[0]}">&nbsp;&nbsp;${i}&nbsp;</a>
                            </li>
                        </c:if>
                        <c:if test="${pb.currentPage != i}">
                            <li>
                                <a href="${pageContext.request.contextPath}/changeStuInfoListServlet?currentPage=${i}&rows=10&ssno=${condition.ssno[0]}&sname=${condition.sname[0]}&smajor=${condition.smajor[0]}">&nbsp;&nbsp;${i}&nbsp;</a>
                            </li>
                        </c:if>
                    </c:forEach>
